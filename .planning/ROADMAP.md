...
- **v2.5 QSO Sorting & Entry Timestamp** — Phases 48–50 (in progress)
...
### v2.5 QSO Sorting & Entry Timestamp (Phases 48–50)
...
**Goal:** Every new QSO inserted through any path carries a `_created_at` UTC timestamp set at document creation time — and the field is protected from being overwritten on subsequent edits.
**Depends on:** Phase 47 (v2.4 complete)
**Requirements:** TS-01, TS-02, TS-03
**Success Criteria** (what must be TRUE):
  1. A QSO inserted via REST API has `_created_at` present in the MongoDB document with a UTC datetime near the insert time — inspectable via MongoDB shell or admin tool
...
**Goal:** `get_qso_page()` validates sort parameters against an allowlist before querying MongoDB, `_created_at` is exposed to templates via the view dict, and the SSE auto-refresh sentinel fires on both default date sort and entry-timestamp sort.
**Depends on:** Phase 48
**Requirements:** SORT-03, SORT-04
**Success Criteria** (what must be TRUE):
  1. Passing an arbitrary field name as the sort parameter (e.g. `?sort=_deleted` or `?sort=hashed_password`) to `get_qso_page()` falls back to the default sort (`-qso_date_utc`) — no arbitrary field is passed to MongoDB
...
**Goal:** Every visible log table column header is clickable to sort, a clock icon in the DATE header provides access to entry-timestamp sort, and all sortable columns display appropriate chevron icons indicating their sort state.
**Depends on:** Phase 49
**Requirements:** SORT-01, SORT-02, UX-01, UX-02
**Success Criteria** (what must be TRUE):
  1. Clicking the MODE column header sorts the log table by MODE ascending; clicking again sorts descending — the sort is reflected in the URL and persists through filter changes
  2. A clock icon is visible inside the DATE column header; clicking it sorts the log table by `_created_at` descending (newest entered first); clicking again sorts ascending — no new visible data column or extra `<td>` appears in any QSO row
