...
| **Estimated runtime** | <10 seconds for source checks; docs build unavailable in this shell |
...
- **After every plan wave:** Run `uv run mkdocs build --strict` when the expected docs toolchain is available.
...
| 65-01-T3 | 01 | 1 | FAV-06, FAV-07 | T-65-03 | Guide source and generated output reference the committed ICO favicon. | generated output | `rg -n 'favicon: assets/favicon.ico' mkdocs.yml`, `rg -n 'rel="icon".*favicon\|favicon.*rel="icon"' site/index.html site/operator-guide/index.html`, `rg -n 'assets/images/favicon\.png' site` | yes | partial |
...
| Gap | Impact | Evidence | Resolution |
|-----|--------|----------|------------|
| `uv run mkdocs build --strict` cannot run in this shell. | Guide favicon source/config and committed generated output are verified, but docs rebuild reproducibility is not proven in this shell. | `uv run mkdocs build --strict` returned `zsh:1: command not found: uv`; `python3 -m mkdocs --version` returned `No module named mkdocs`; `.venv/bin/mkdocs --version` has a stale interpreter path. | Run `uv run mkdocs build --strict` in an environment with `uv` installed or repair the repo virtualenv, then update this validation to compliant if it passes. |
...
| Automated/source checks green | 6 |
...
**Approval:** partial 2026-06-13; blocked only by unavailable MkDocs/uv rebuild tooling.
